 ...
 */
?>
<article id="post-<?php the_ID(); ?>" class="single-film">
	...
	<footer class="entry-meta">
		<?php
        //Here I am trying to display the meta datas related to the film cpt. But first we have to be sure that data are from a film CPT
        if ( 'films' == get_post_type() ) { ?>
            <div class="entry-meta">
                <div class="metas">Country: <?php echo films_get_terms( $post->ID, 'country' ); ?></div>
                <div class="metas">Genre: <?php echo films_get_terms( $post->ID, 'genre' ); ?></div>
                <div class="metas">Ticket Price : <a href="#"><?php echo get_field( 'ticket_price' ); ?>$</a></div>
                <div class="metas">Release Date: <a href="#"><?php echo get_field( 'release_date' ); ?></a></div>
            </div><!-- .entry-meta -->
	    <?php
        } else {
